// Vanila JavaScr todo list app
var taskInput = document.getElementById("new-task");
// ye <input> field wali id(The id attribute is most used to point to a style in a style sheet, and by JavaScript (via the HTML DOM) to manipulate the element with the specific id.) hai mean input field m jo b ho taskinput m aa jae
//(The Document method getElementById() returns an Element object representing the element whose id property matches the specified string. Since element IDs are required to be unique if specified, they're a useful way to get access to a specific element quickly.)


var addButton = document.getElementsByTagName("button")[0]; // First button(add button to varaddButton m dala h)
var incompleteTasksHolder = document.getElementById("incomplete-tasks");  // Incomplete-tasks ko access kiya h
...
var createNewTaskElement = function (taskString) {       // New Task List Item
  ...
  var label = document.createElement("label");          // Label(mean name o item like <label>Go Shopping</label>)
